	...
	SubShader {
		...
		Pass {
			...
			float4 frag (v2f i) : SV_Target {
				...
				// - Reflections -
				half3 reflection = max(probeColor, tex2D(_SpecularRamp, spec_uv));
				lightIntensity = lerp(lightIntensity + reflection * (1-rma.r), reflection, rma.g * (1-rma.r));
				// - AO -
				lightIntensity *= rma.b;

				// --- Specular ---
				float specularIntensity = (pow(NdotH * lightIntensity, _Glossiness * _Glossiness) /*NdotL*/ * shadow);
