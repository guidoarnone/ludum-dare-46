Shader "Toon/SSS" {
	...
	Properties {
		...
		_ThickTex("Thickness Texture", 2D) = "white" {}

		_Distortion("SSS Distortion", float) = 0.001
		_Power("SSS Power", float) = 4
		_Scale("SSS Scale", float) = 0.5
		_Attenuation("SSS Attenuation", float) = 1

		_SpecularRamp("Specular ramp texture", 2D) = "black" {}
		_SpecularHigh("Specular Upper Bound", float) = 1
		_SpecularLow("Specular Lower Bound", float) = 0.8
		[HDR]
	...
	SubShader {
		...
		Pass {
			...
			float4 frag (v2f i) : SV_Target {
				...
				float4 thick = tex2D(_ThickTex, i.uv);
				...
				// - Reflections -
				half3 reflection = max(probeColor, tex2D(_SpecularRamp, spec_uv) * lightIntensity);
				...
				float specularIntensitySmooth = smoothstep(_SpecularLow / 100, _SpecularHigh / 100, specularIntensity * (1-rma.r));
				// - Metal/Highlights -
				...
				float4 I = _Attenuation * (VdotH + _AmbientColor) * thick;
				...
				float4 SS = I * _LightColor0;
				
				//return shadow;
				return color *  _Color * (_AmbientColor + light + specular + rim + SS);
			}
			ENDCG
		}
    }
