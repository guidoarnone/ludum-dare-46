	...
	SubShader {
		Pass {
			...
			v2f vert (float4 vertex : POSITION, float3 normal : NORMAL, float2 uv : TEXCOORD0) {
				vertex = rotate_around_y(vertex, _Time[3]);
				normal = rotate_around_y(float4(normal.xyz, 0), _Time[3]);
				v2f o;
		...
		Pass {
            ...
            v2f vert(appdata_base v) {
                v2f o;
				v.vertex = rotate_around_y(v.vertex, _Time[3]);
				v.normal = rotate_around_y(float4(v.normal.xyz, 0), _Time[3]);
